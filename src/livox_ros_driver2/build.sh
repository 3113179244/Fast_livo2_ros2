...
if [ $ROS_VERSION = ${VERSION_ROS1} ]; then
    ...
    cd ../../
    colcon build --packages-ignore sophus --cmake-args -DCMAKE_BUILD_PARALLEL_LEVEL=4 -DBUILD_TESTING=OFF -DBUILD_TESTS=OFF -DROS_EDITION=${VERSION_ROS2} -DDISTRO_ROS=${ROS_DISTRO}
fi
popd > /dev/null
